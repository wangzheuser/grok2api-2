...
class SqlManagedProxyStateRepository:
    """基于 SQLAlchemy 事务的 托管代理共享状态仓储。"""
            ...
            async with self._engine.begin() as conn:
                ...
                if self._dialect == "mysql":
                    ...
                    if not mysql_lock_acquired:
                        raise RuntimeError(
                            "timed out acquiring managed proxy schema lock"
                        )
                else:
                    await conn.execute(
                    ...
                    await conn.execute(
                        self._insert_ignore(
                            runtime_table,
                            {
                                ...
                                "health_state": ProxyHealthState.UNKNOWN.value,
                                "checking": False,
                                "runtime_epoch": 0,
                                "last_error": "",
        ...
        async with self._engine.begin() as conn:
            result = await conn.execute(
                ...
                .where(
                    ...
                    job_table.c.status == ProxyHealthJobStatus.RUNNING.value,
    ...
    async def complete_health_job_item(
        ...
        *,
        ...
        await self.initialize()
        counter = {
            ProxyProbeOutcome.HEALTHY: job_table.c.healthy,
            ProxyProbeOutcome.UNHEALTHY: job_table.c.unhealthy,
            ProxyProbeOutcome.INCONCLUSIVE: job_table.c.inconclusive,
            ProxyProbeOutcome.SKIPPED: job_table.c.skipped,
        }[outcome]
        async with self._engine.begin() as conn:
            result = await conn.execute(
        ...
        *,
        ...
        async with self._engine.begin() as conn:
            result = await conn.execute(
                ...
                .values(
                    status=(
                        ProxyHealthJobStatus.FAILED.value
                        if error
                        else ProxyHealthJobStatus.COMPLETED.value
                    ),
                    updated_at=timestamp_ms,
...
def _runtime_values(record: ProxyRuntimeRecord) -> dict[str, Any]:
    """把运行态转换为 SQL values。"""
    return {
...
def _runtime_from_mapping(row: Any) -> ProxyRuntimeRecord:
    """把 SQL 行转换为运行态模型。"""
...
def _job_values(job: ProxyHealthJob) -> dict[str, Any]:
    """把健康任务转换为 SQL values。"""
    return {
        "job_id": job.job_id,
        "kind": job.kind.value,
...
def _job_from_mapping(row: Any) -> ProxyHealthJob:
    """把 SQL 行转换为健康任务。"""
...
def _job_item_from_mapping(row: Any) -> ProxyHealthJobItem:
    """把 SQL 行转换为健康任务节点。"""
...
__all__ = ["SqlManagedProxyStateRepository"]
